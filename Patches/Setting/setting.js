        ...
        const SECTIONS = [
            {
                name: 'Refresh Rate', priority: 1, settings: [
                    { type: Ui.ComponentType.Toggle, id: 'EnableRefreshRate', name: 'Enable', description: 'Overriding the XSOverlay render refresh rate.', default: false },
            ...
            },
            {
                name: 'Cursor', priority: 2, settings: [
                    { type: Ui.ComponentType.Toggle, id: 'AlwayUpdateCursor', name: 'Always Update Cursor', description: 'Reduces cursor latency by sending cursor position data from the Pointer before the desktop frame is captured.<br>Without this, the cursor often appears to lag one frame behind the Pointer position.', default: true },
                    ...
                    { type: Ui.ComponentType.Toggle, id: 'PhysicalMouseDetector', name: 'Physical Mouse Detector', description: 'Release the Pointer control when physical mouse movement is detected.<br>Pointer click to regain control.', default: true }
                ]
            },
            {
                name: 'Pointer', priority: 3, settings: [
                    ...
                    { type: Ui.ComponentType.Slider, id: 'PointerScaleMultiply', name: 'Scale Multiplier', description: 'Multiplier for the Pointer scale relative to the XSOverlay setting.', default: 100, options: [100, 1000, 50], unit: '%' },
                    { type: Ui.ComponentType.Toggle, id: 'PointerDoubleClickDelay', name: 'Double Click Delay', description: 'Apply a Double Click Delay from XSOverlay setting to the Pointer itself, not just the cursor.', default: true }
                ]
            },
            {
                name: 'Mouse Navigation', priority: 4, settings: [
                    { type: Ui.ComponentType.Toggle, id: 'MouseNavigation', name: 'Enable', description: 'Custom keybindings for Mouse Forward/Back navigation.<br>Configuration by press \"Bindings\" tab in XSOverlay settings to open SteamVR bindings menu.<br>Edit the Current Binding and add a button for \"MouseBack/MouseForward\".', default: true },
                    { type: Ui.ComponentType.Toggle, id: 'MouseNavigationUseModifiedKey', name: 'Use Alt+Left/Right', description: 'Use Alt+Left/Right keyboard shortcuts for navigation instead of mouse clicks.<br>Targets the focused window instead of the hovered window.', default: false }
                ]
            }
